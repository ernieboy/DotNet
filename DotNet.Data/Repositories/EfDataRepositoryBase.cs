...
using DotNet.Data.EntityFramework.Extensions;
...
namespace DotNet.Data.Repositories
{
        ...
        where TContext : DbContext, new()
    {
        ...
        public async Task<TEntity> FindEntityByStringId(string id)
        {
            ...
            if (!CurrentEntityHasIntPropertyWithKeyAttribute(out keyColumnName))
            {
                throw new DataAccessException(
                    "Entity does not contain any property that is marked with the KEY attribute!");
            }
        ...
        private bool CurrentEntityHasIntPropertyWithKeyAttribute(out string propertyName)
        {
            ...
            PropertyInfo keyMember =
                entityType.GetProperties().FirstOrDefault(p => p.GetCustomAttributes<KeyAttribute>().Any());
